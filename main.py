...
import telebot, threading, schedule, sqlite3, sys, getopt
...
def main(argv):
    global TOKEN, DB_LOC, ADMIN_ID
    try:
        opts, args = getopt.getopt(argv, "t:d:a:", ["token=", "database=", "admid="])
    except getopt.GetoptError:
        print('main.py -t <token> -d <database_location> -a <admin_id>')
        sys.exit(2)
    for opt, arg in opts:
        if opt in ("-t", "--token"):
            TOKEN = arg
...
while 1:
    ...
    def send_welcome(message):
        bot.reply_to(message,
                     f'Я бот. Приятно познакомиться, {message.from_user.first_name}, your id is {message.from_user.id}')
    ...
    def adduser(message):
        if message.from_user.id == ADMIN_ID:
            ...
            curcon = contr.cursor()
            commandl = message.text.split()
            curcon.execute("INSERT INTO userdata VALUES(?, ?, ?, ?, ?)",
                           (commandl[1], commandl[2], commandl[3], commandl[4], commandl[5],))
            contr.commit()
        ...
        if message.text.lower() == 'инфа':
            ...
            for num in range(0, len(users)):
                if users[num].muid == message.from_user.id:
                    iffound += 1
                    bot.send_message(message.from_user.id,
                                     users[num].gs(users[num].login, users[num].passw, users[num].isp))
            if iffound == 0:
